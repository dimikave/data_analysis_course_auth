...
% Exercise 8 but with cross validation
...
for t = 0:30
    ...
    if t>0
        ...
        mse(j,1) = crossval('mse', X(:,2:end), Y,'Predfun',fcn, 'kfold',5);
    else
        AdjR2Array(j,1)= mdl.Rsquared.Adjusted;
        mse(j,1) = crossval('mse', X, Y,'Predfun',fcn, 'kfold',5);
    end
    ...
    if t>0
        ...
        mse(j,2) = crossval('mse', X(:,model), Y,'Predfun',fcn, 'kfold',5);
...
variableNames = {'R_Squared_Ordinary','lag_Ordinary_Rsq','Adjusted_R_Squared_Ordinary','lag__Ordinary_Adj','min_MSE_Ordinary','lag_MSE_Ordinary','R_Squared_StepWise','lag_StepWise_Rsq','Adjusted_R_Squared_StepWise','lag_StepWise_Adj','min_MSE_SW','lag_MSE_SW'};
rowNames = {'Period_1','Period_2'};
arr = [r(1,1) ind(1,1) a(1,1) i(1,1) er(1,1) in(1,1) r(1,2) ind(1,2) a(1,2) i(1,2) er(1,2) in(1,2);...
    r(2,1) ind(2,1) a(2,1) i(2,1) er(2,1) in(2,1) r(2,2) ind(2,2) a(2,2) i(2,2) er(2,2) in(2,2)];
...
% between performance and complexity. Moreover, comparing with Exercise 8,
% the results seem to agre with the results before cross validation.